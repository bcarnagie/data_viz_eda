...
weather_df = 
  ...
  mutate(
    ...
    tmin = tmin / 10,
    tmax = tmax / 10,
    month = lubridate::floor_date(date, unit = "month")) |> # months get rounded down to the lowest unit in this function (january 1-> january 31)
  select(name, id, everything())
```

    ## using cached file: /Users/briannacarnagie/Library/Caches/org.R-project.R/R/rnoaa/noaa_ghcnd/USW00094728.dly
...
here are the big outlier, knowing they exist is really important
...
``` r
weather_df |> # untidied the dataset
  count(name, month) |> 
  pivot_wider(
    names_from = name,
    values_from = n
...
weather_df |> 
  ...
  ggplot(aes(x = month, y = mean_tmax, color = name)) + #easier to interpret view
  geom_point() + 
  geom_line()
```
